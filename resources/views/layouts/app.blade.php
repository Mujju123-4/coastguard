...
>>>>>>> 89a3400d8febfa7c0af4cd0221386851a7d4c933
                      ...
                      <a href="{{ route('contact_details') }}" class="flex items-center px-4 py-3 {{ request()->routeIs('user-manuals.*') ? 'bg-orange-600/20 text-orange-400 border border-orange-500/30' : 'text-slate-300 hover:bg-white/5 hover:text-white' }} rounded-lg transition-colors">
                        <svg class="w-5 h-5 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253" />
                        </svg>
                      Contact details 
...
>>>>>>> 89a3400d8febfa7c0af4cd0221386851a7d4c933
                        ...
                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                            <div @click="open = !open" class="flex items-center space-x-3 cursor-pointer hover:bg-slate-50 p-1.5 rounded-xl transition-colors select-none">
                                <div class="text-sm font-medium text-slate-600 hidden sm:block">{{ Auth::user()->name }}</div>
                            ...
                            <div x-show="open" 
                                ...
                                <a href="{{ route('profile.edit') }}" class="flex items-center px-4 py-2.5 text-sm text-slate-600 hover:bg-slate-50 hover:text-orange-600 transition-colors">
...
<<<<<<< HEAD
        ...
        var IS_ADMIN     = {{ auth()->user()->hasAnyRole(['Admin', 'Superadmin', 'Super Admin']) ? 'true' : 'false' }};
        var CAN_SEND_MSG = {{ auth()->user()->hasAnyRole(['Super Admin', 'Location User', 'Location Users']) ? 'true' : 'false' }};


        // If local variable isn't found, default to user creation date to avoid showing everything ever created
        var defaultTs       = '{{ auth()->user()->created_at ? auth()->user()->created_at->toISOString() : "1970-01-01T00:00:00Z" }}';
