...
Route::get('/dashboard', function () {
    $user = auth()->user();
    $isAdmin = $user->hasRole(['Admin','Superadmin','Super Admin']);
    ...
    $locationBreakdownQuery = \App\Models\Location::withCount(['items', 'users']);
    if (!$isAdmin) {
        $locationBreakdownQuery->where('id', $user->location_id);
    }
    $locationBreakdowns = $locationBreakdownQuery->get();

    $ticketStats = null;
    $isAdminRole = $user->hasAnyRole(['Admin', 'Superadmin', 'Super Admin']);
    if ($isAdminRole) {
     
        $ticketStats = [
            'total' => \App\Models\Ticket::count(),
            'pending' => \App\Models\Ticket::where('status', 'open')->count(),
            'resolved' => \App\Models\Ticket::where('status', 'closed')->count(),
        ];
    }
     else{
        
         $ticketStats = [
            'total' => \App\Models\Ticket::where('raised_by',$user->id)->count(),
            'pending' => \App\Models\Ticket::where('raised_by',$user->id)->where('status', 'open')->count(),
            'resolved' => \App\Models\Ticket::where('raised_by',$user->id)->where('status', 'closed')->count(),
        ];
    ...
    return view('dashboard', compact('notices', 'stats', 'locationBreakdowns', 'ticketStats', 'isAdminRole'));
})->middleware(['auth', 'verified'])->name('dashboard');

Route::middleware('auth')->group(function () {
    Route::get('/profile', [ProfileController::class, 'edit'])->name('profile.edit');
        Route::get('/contact-details', [LocationController::class, 'contactDetails'])->name('contact_details');
    Route::patch('/profile', [ProfileController::class, 'update'])->name('profile.update');
    Route::delete('/profile', [ProfileController::class, 'destroy'])->name('profile.destroy');
Route::middleware(['role:admin|Super Admin|Parallel Admin'])->group(function () {
    ...
    Route::resource('notice-board', NoticeController::class)->names('notices')->parameters(['notice-board' => 'notice']);
